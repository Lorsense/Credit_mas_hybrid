...
def build_wg_ids(config):
    ...
    # Colocated engines need one non-overlapping port range per worker group.
    for agents_configs in wg_to_agents.values():
        for ac in agents_configs:
            ac["config_actor_rollout_ref"].rollout["agent_count"] = len(wg_to_agents)
    return wg_to_agents
