...
from verl.models.transformers.qwen3_5 import (
    configure_qwen3_5_native, get_text_config, is_qwen3_5,
    prepare_qwen3_5_for_text_training, select_auto_model_class, validate_qwen3_5_execution,
    validate_qwen3_5_loading_info,
)
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        native_qwen3_5 = is_qwen3_5(actor_model_config)
        if native_qwen3_5:
            validate_qwen3_5_execution(
                actor_model_config, use_remove_padding=use_remove_padding,
                ulysses_sp_size=self.ulysses_sequence_parallel_size,
                use_fused_kernels=use_fused_kernels, use_liger=use_liger,
                lora_rank=self.config.model.get("lora_rank", 0),
            )
            # Keep master weights, especially GatedDeltaNet A_log/dt_bias, in
            # FP32 from loading onward. Recasting BF16 weights cannot restore
            # precision. Reference-policy loading follows the same rule.
            torch_dtype = torch.float32
        attention_backend = self.config.model.get("attn_implementation") or ("sdpa" if native_qwen3_5 else "flash_attention_2")
        actor_model_config._attn_implementation = attention_backend
        ...
        if native_qwen3_5:
            # Token IDs live in text_config on official conditional checkpoints.
            for token_key in ("bos_token_id", "eos_token_id", "pad_token_id"):
                setattr(actor_model_config.text_config, token_key, override_config_kwargs[token_key])
            configure_qwen3_5_native(actor_model_config, attn_implementation=attention_backend)
        if self.rank == 0:
            print(f"Model config after override: {actor_model_config}")

        # NOTE(fix me): tie_word_embedding causes meta_tensor init to hang
        init_context = get_init_weight_context_manager(use_meta_tensor=not get_text_config(actor_model_config).tie_word_embeddings, mesh=self.device_mesh)

        with init_context(), warnings.catch_warnings():
            warnings.simplefilter("ignore")
            actor_module_class = select_auto_model_class(actor_model_config)
            ...
            actor_module = actor_module_class.from_pretrained(
                ...
                output_loading_info=native_qwen3_5,
            )
            if native_qwen3_5:
                actor_module, loading_info = actor_module
                validate_qwen3_5_loading_info(loading_info)
            ...
            if native_qwen3_5:
                prepare_qwen3_5_for_text_training(actor_module)
        ...
        if fsdp_strategy == "fsdp":
            actor_module_fsdp = FSDP(
                ...
                # Frozen vision and trainable text coexist in the full official
                # checkpoint; FSDP1 must support mixed requires_grad parameters.
                use_orig_params=native_qwen3_5,
    ...
    def init_model(self):
        ...
        if self._is_actor:
            self.flops_counter = None if is_qwen3_5(self.actor_model_config) else FlopsCounter(self.actor_model_config)
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if is_qwen3_5(self.actor_model_config):
                # The legacy FLOPs estimator assumes pure attention layers and
                # cannot measure hybrid GatedDeltaNet/dense-or-MoE models.
                metrics[f"perf/{wg_id}/mfu_supported"] = 0.0
            else:
                estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
                metrics[f"perf/{wg_id}/mfu_supported"] = 1.0
