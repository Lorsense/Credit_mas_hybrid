...
except ImportError:  # sglang >= 0.5.x renamed get_ip
    from sglang.srt.utils import get_open_port
    from sglang.srt.utils import get_local_ip_by_remote as get_ip
...
from verl.utils.entropy_credit import compute_action_topk_entropy
from verl.utils.sglang_weight_sync import validate_local_tp_topology
...
def _engine_runtime_kwargs(config, model_hf_config):
    """Apply rollout limits to Engine and protect distributed synchronization."""
    options = {
        "context_length": int(config.max_model_len),
        "disable_cuda_graph": bool(config.enforce_eager),
    }
    extras = (config.get("engine_kwargs") or {}).get("sglang", {}) or {}
    if OmegaConf.is_config(extras):
        extras = OmegaConf.to_container(extras, resolve=True)
    protected = {
        "model_path", "dtype", "mem_fraction_static", "enable_memory_saver",
        "base_gpu_id", "gpu_id_step", "tp_size", "node_rank", "load_format",
        "dist_init_addr", "nnodes", "trust_remote_code", "port",
        "context_length", "disable_cuda_graph", "custom_weight_loader",
    }
    for name, value in extras.items():
        if value is None:
            continue
        if name in protected:
            raise ValueError(f"Set SGLang {name} through the rollout configuration, not engine_kwargs")
        options[name] = value
    if is_qwen3_5(model_hf_config):
        if options.get("quantization") is not None or options.get("enable_lora", False):
            raise ValueError("Qwen3.5 rollout requires non-quantized weights and no LoRA")
        options["custom_weight_loader"] = ["verl.utils.sglang_weight_sync.strict_load_weights"]
    return options


def _engine_port(config, rank):
    local_rank = int(os.environ.get("RAY_LOCAL_RANK", os.environ.get("LOCAL_RANK", rank)))
    agent_index = int(config.agent_port)
    agent_count = int(config.get("agent_count", 2))
    if local_rank < 0 or not 0 <= agent_index < agent_count:
        raise ValueError("Invalid node-local rank or rollout worker-group port index")
    # SGLang draws its NCCL port from base+[100,1000]. Separate 1024-wide
    # ranges avoid check-then-bind races for every colocated engine, including
    # the default 16-GPU node (the former rank%8 scheme collided on rank 8).
    port = 10000 + (local_rank * agent_count + agent_index) * 1024
    if port + 1000 > 65535:
        raise ValueError("Too many colocated rollout engines for reserved port ranges; reduce per-node workers")
    return port


def _function_call_parser_class():
    # Pure-text Math does not depend on SGLang's changing tool API.
    try:
...
def _post_process_outputs(tokenizer, output, include_finish_reason=False):
    def _map_each_response(resp):
        ...
        if include_finish_reason:
            finish_reason = resp.get("meta_info", {}).get("finish_reason", {})
            if isinstance(finish_reason, dict):
                finish_reason = finish_reason.get("type", "unknown")
            return torch.tensor(output_token_ids), torch.tensor(log_probs), str(finish_reason or "unknown")
    ...
    batched_logprobs = []
    finish_reasons = [] if include_finish_reason else None
    for result in out_map:
        output_token_ids, log_probs = result[:2]
        batched_output_token_ids.append(output_token_ids)
        batched_logprobs.append(log_probs)
        if include_finish_reason:
            finish_reasons.append(result[2])
    pad_token_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id
    batched_output_token_ids = pad_sequence(batched_output_token_ids, batch_first=True, padding_value=pad_token_id)
    if len(batched_logprobs) > 0:
        batched_logprobs = pad_sequence(batched_logprobs, batch_first=True, padding_value=pad_token_id)
    if include_finish_reason:
        return batched_output_token_ids, batched_logprobs, finish_reasons
    return batched_output_token_ids, batched_logprobs


def _attach_action_topk_entropy(output, top_k: int) -> None:
    """Keep compact normalized Top-K action entropy, not token distributions."""
    if top_k <= 0:
        return
    responses = output if isinstance(output, list) else [output]
    for response in responses:
        meta_info = response.setdefault("meta_info", {})
        output_token_logprobs = meta_info.get("output_token_logprobs", [])
        meta_info["top16_entropy"] = compute_action_topk_entropy(
            meta_info.get("output_top_logprobs"),
            response_mask=[True] * len(output_token_logprobs),
            top_k=top_k,
        )
...
def get_tool_call_parser_type(tokenizer: PreTrainedTokenizer) -> str:
    FunctionCallParser = _function_call_parser_class()
...
class SGLangRollout(BaseRollout):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.config = config
        self.top_logprobs_num = int(config.get("top_logprobs_num", 0) or 0)
        if self.top_logprobs_num not in (0, 16):
            raise ValueError("top_logprobs_num must be 0 (disabled) or 16")
        ...
        assert not (not config.enforce_eager and config.free_cache_engine), "free_cache_engine requires enforce_eager=True"
    ...
    def _init_distributed_env(self, device_mesh_cpu, **kwargs):
        ...
        if self.train_tp is None:
            # Check local IPC topology before creating any expensive engines.
            topology = [None] * dist.get_world_size()
            dist.all_gather_object(topology, (socket.gethostname(), self._device_mesh_cpu["tp"].mesh.tolist()))
    ...
    def _verify_config(self, model_hf_config):
        ...
        assert self.config.max_model_len >= self.config.prompt_length + self.config.response_length, f"""max_model_len should be greater than total sequence length (prompt_length + response_length): 
            {self.config.max_model_len} >= {self.config.prompt_length} + {self.config.response_length}"""
        assert get_text_config(model_hf_config).max_position_embeddings >= self.config.max_model_len, "model context length should be greater than total sequence length"
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if first_rank_in_node:
            ...
            self._engine = Engine(
                ...
                port=_engine_port(self.config, rank),
                **_engine_runtime_kwargs(self.config, self.model_hf_config),
    ...
    def _init_sampling_params(self, **kwargs):
        ...
        for k in self.config.keys():
            if k == "top_logprobs_num":
                # Engine request option, not a SamplingParams field.
                continue
        ...
        with self.update_sampling_params(**kwargs):
            ...
            if self._tp_rank == 0:
                ...
                if self.top_logprobs_num > 0:
                    engine_kwargs["top_logprobs_num"] = self.top_logprobs_num
                output = loop.run_until_complete(self._engine.async_generate(**engine_kwargs))
                _attach_action_topk_entropy(output, self.top_logprobs_num)
            else:
                output = None
            ...
            capture_event_trace = prompts.meta_info.get("capture_event_trace", False)
            out = _post_process_outputs(self.tokenizer, output, include_finish_reason=capture_event_trace)

            response = out[0].to(idx.device)
            rollout_log_probs = out[1].to(idx.device)
            generation_finish_reasons = out[2] if capture_event_trace else None
            ...
            if capture_event_trace:
                _non_tensor_batch["generation_finish_reason"] = np.array(generation_finish_reasons, dtype=object)
            if self.top_logprobs_num > 0:
                top16_entropy_stats = [response["meta_info"].get("top16_entropy") for response in output]
                _non_tensor_batch["top16_entropy"] = np.asarray(top16_entropy_stats, dtype=object)
                _non_tensor_batch["top16_entropy_mean"] = np.asarray(
                    [np.nan if stats is None else float(stats["mean"]) for stats in top16_entropy_stats],
                    dtype=np.float64,
                )
