...
class RayResourcePool(ResourcePool):
    ...
    def get_placement_groups(self, strategy="STRICT_PACK", name=None, device_name="cuda"):
        ...
        # Reserve larger node bundles first, and wait for each reservation.
        # With 7+8 free GPUs (one scorer already occupies the first node),
        # scheduling the 7-GPU group first can consume the only 8-GPU node.
        # Merely submitting both in size order still permits that race.
        # Preserve the original pool order for callers and checkpoint ranks.
        pgs = [None] * len(pg_scheme)
        for idx in sorted(range(len(pg_scheme)), key=lambda index: len(pg_scheme[index]), reverse=True):
            pg = placement_group(bundles=pg_scheme[idx], strategy=strategy, name=pg_name_prefix + str(idx), lifetime=lifetime)
            ray.get(pg.ready())
            pgs[idx] = pg
...
class RayWorkerGroup(WorkerGroup):
    ...
    def _init_with_resource_pool(self, resource_pool, ray_cls_with_init, bin_pack, detached):
        ...
        rank = -1
        # Sorting by actual node IP can reorder heterogeneous placement groups.
        # Bind each requested local size to its group before sorting; reused
        # groups may contain more bundles than this pool requested.
        if len(pgs) != len(resource_pool.store):
            raise ValueError("Each Actor node requires one placement group")
        local_sizes = {pg.id: count for pg, count in zip(pgs, resource_pool.store)}
        for pg_idx, pg in enumerate(sort_placement_group_by_node_ip(pgs)):
            local_world_size = local_sizes[pg.id]
            assert local_world_size <= pg.bundle_count, f"insufficient bundles for {self.name_prefix} node {pg_idx}"
