...
from verl.models.transformers.qwen3_5 import install_qwen3_5_padding_fix, is_qwen3_5, validate_qwen3_5_execution
...
def _ulysses_flash_attention_forward(
    ...
    **kwargs,
    ...
    """
    # Padded native HF models do not need the legacy FlashAttention API at
    # import time (and do not need the optional flash-attn package installed).
...
def apply_monkey_patch(
    ...
    if is_qwen3_5(model.config):
        validate_qwen3_5_execution(
            model.config, use_remove_padding=use_remove_padding,
            ulysses_sp_size=ulysses_sp_size, use_fused_kernels=use_fused_kernels,
        )
        install_qwen3_5_padding_fix(model.config)
        return
    if not use_remove_padding and ulysses_sp_size == 1 and not use_fused_kernels:
        return
    if use_fused_kernels and is_transformers_version_in_range(min_version="5.0.0"):
        raise ValueError("Legacy fused PPO forward is unsupported on Transformers 5; set use_fused_kernels=False")
