本目录已合入 **zxj Math LOTO／GAE 优势、mix 两阶段信用分配、纯语义价值驱动的独立熵约束**，并移除零优势恢复。方法说明、语义预训练、单机 16 卡／双机 8 卡启动及恢复方式见 [Hybrid 使用说明](docs/hybrid_method.md)；训练入口为 [run_math_hybrid.sh](examples/drmas_trainer/run_math_hybrid.sh)。
